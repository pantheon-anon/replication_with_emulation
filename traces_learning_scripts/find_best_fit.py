...
def best_fit_distribution(data, bins=200, ax=None):
    ...
    DISTRIBUTIONS = [
        ...
        st.laplace,st.levy,st.levy_l,st.levy_stable,st.logistic,st.loggamma,
        st.loglaplace,st.lognorm,st.lomax,st.maxwell,st.mielke,st.nakagami,
        st.ncx2,st.ncf,st.nct,st.norm,st.pareto,st.pearson3,st.powerlaw,
    ...
    for distribution in DISTRIBUTIONS:
        ...
        try:
            ...
            with warnings.catch_warnings():
                ...
                if not np.isnan(sse):
                    results.append((distribution.name, sse))
...
def find_best_fit(data):
    ...
    ax.hist(data, bins=50, normed=True, alpha=0.5,
    ...
    fig.savefig('fit_continuous_distributions.pdf')
